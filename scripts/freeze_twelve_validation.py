...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Freeze 30-minute Twelve Data references for the fixed June 1-30, 2026 "
            "validation run."
        )
    )
...
def main() -> int:
    ...
    if not api_key:
        raise SystemExit("TWELVE_DATA_API_KEY is required in backend/.env")
    ...
    try:
        run_path = TwelveReferencePipeline(
            client,
    ...
    print(f"Frozen Twelve Data validation run: {run_path}")
    print("API requests used: 5 (one candle request and four indicator requests)")
    return 0
